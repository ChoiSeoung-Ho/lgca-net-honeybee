...
def save_fold_visualizations(image_path, saliencies, save_dir, model_name, subclass):
    ...
    plt.suptitle(f"{model_name} Saliency Comparison (Fold1~3)", fontsize=14)

    for i in range(3):
        saliency_np = saliencies[i].squeeze().cpu().numpy()
        axes[i].imshow(img_np)
        # vmin, vmax를 None으로 설정하여 실제 값 범위 사용
        axes[i].imshow(saliency_np, cmap='jet', alpha=0.5, vmin=None, vmax=None)
        axes[i].set_title(f"Saliency Fold {i+1}")
        axes[i].axis('off')

    plt.tight_layout()
    compare_path = os.path.join(img_dir, f"{base_name}_{model_name}_saliency_folds_compare.png")
    plt.savefig(compare_path, bbox_inches="tight")
    plt.close()

    avg_saliency = torch.mean(torch.stack(saliencies), dim=0)

    fig, ax = plt.subplots(1, 1, figsize=(6, 6))
    avg_saliency_np = avg_saliency.squeeze().cpu().numpy()
    ax.imshow(img_np)
    # vmin, vmax를 None으로 설정하여 실제 값 범위 사용
    ax.imshow(avg_saliency_np, cmap='jet', alpha=0.5, vmin=None, vmax=None)
    ax.set_title("Average Saliency")
    ax.axis('off')
    avg_path = os.path.join(img_dir, f"{base_name}_{model_name}_saliency_average.png")
    plt.tight_layout()
        ...
        for idx, (img_path, subclass) in enumerate(selected_paths):
            ...
            try:
                ...
                if len(saliencies) > 0:
                    ...
                    save_fold_visualizations(img_path, saliencies, result_path, model_name, subclass)
...
if __name__ == "__main__":
    ...
    print(f"Starting Saliency Map generation for {model_name}")
